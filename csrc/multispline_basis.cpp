...
multispline_basis_fw(torch::Tensor pseudo, torch::Tensor kernel_size,
                ...
                torch::Tensor resolution, int64_t log2_hashmap_size, int64_t cellsize, torch::Tensor xyz, torch::Tensor point_index, torch::Tensor primes, torch::Tensor offsets) {
  if (pseudo.device().is_cuda()) {
    
#ifdef WITH_CUDA
    return multispline_basis_fw_cuda(pseudo, kernel_size, is_open_spline, degree, resolution, log2_hashmap_size, cellsize, xyz, point_index, primes, offsets);
...
public:
  static variable_list forward(AutogradContext *ctx, Variable pseudo,
                               ...
                               int64_t degree, Variable resolution, int log2_hashmap_size, int cellsize, Variable xyz, Variable point_index, Variable primes, Variable offsets) {
    ctx->saved_data["degree"] = degree;
    auto result = multispline_basis_fw(pseudo, kernel_size, is_open_spline, degree, resolution, log2_hashmap_size, cellsize, xyz, point_index, primes, offsets);
...
multispline_basis(torch::Tensor pseudo, torch::Tensor kernel_size,
             torch::Tensor is_open_spline, int64_t degree, torch::Tensor resolution, int64_t log2_hashmap_size, int64_t cellsize, torch::Tensor xyz, torch::Tensor point_index,
             torch::Tensor primes, torch::Tensor offsets) {
  pseudo = pseudo.contiguous();
  auto result = SplineBasis::apply(pseudo, kernel_size, is_open_spline, degree, resolution, log2_hashmap_size, cellsize, xyz, point_index, primes, offsets);
