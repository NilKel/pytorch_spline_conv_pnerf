#include "confidence_basis_cuda.h"
...
//
// Forward kernel for confidence basis generation.
// This kernel computes two outputs:
//   - basis: the interpolated confidence basis value (a product of linear weights)
//   - weight_index: the contiguous index into the confidence feature table,
//     computed as: 
//         base_index + multi-index,
//     where base_index = point_idx * (∏_{d} kernel_size[level*D+d])
// and the multi-index is computed from the pseudo coordinates.
//
template <typename scalar_t, int64_t degree>
__global__ void
confidence_basis_fw_kernel(const scalar_t *pseudo, const int64_t *kernel_size,
                           const uint8_t *is_open_spline, scalar_t *basis,
                           int64_t *weight_index, int64_t E, int64_t D, int64_t S,
                           int64_t numel, int64_t levels, const int64_t *resolution,
                           int64_t cellsize, const scalar_t *xyz, const int64_t *point_index,
  ...
  if (thread_idx < numel) {
    ...
    for(int64_t level = 0; level<levels; level++){
      ...
      unsigned int temp = (wi + point_idx * offsets[level]);
...
confidence_basis_fw_cuda(torch::Tensor pseudo, torch::Tensor pseudo_unscaled,
                         torch::Tensor kernel_size, torch::Tensor is_open_spline,
                         int64_t degree, torch::Tensor resolution,int64_t log2_hashmap_size, int64_t cellsize,
                         torch::Tensor xyz, torch::Tensor point_index,
                         torch::Tensor primes,
                         torch::Tensor offsets) {
  // Note: 'pseudo_unscaled' is a dummy input to enable gradient flow; it is not used in computation.
  CHECK_CUDA(pseudo);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(pseudo.scalar_type(), "confidence_basis_fw", [&] {
    ...
    AT_DISPATCH_DEGREE_TYPES(degree, [&] {
      confidence_basis_fw_kernel<scalar_t, DEGREE>
          <<<BLOCKS(E * S), THREADS, 0, stream>>>(
              pseudo_data, kernel_size_data, is_open_spline_data, basis_data,
              weight_index_data, E, D, S, E * S, levels, resolution_data,
              cellsize, xyz_data, point_index_data, offsets_data);
    });
  });
...
//
// Backward kernel for confidence basis generation.
// (Here we simply reuse a similar structure to your previous backward kernel.)
//
template <typename scalar_t, int64_t degree>
__global__ void
confidence_basis_bw_kernel(const scalar_t *grad_basis, const scalar_t *pseudo,
                           const int64_t *kernel_size, const uint8_t *is_open_spline,
                           scalar_t *grad_pseudo, int64_t E, int64_t D, int64_t S,
                           int64_t numel, int64_t levels) {
  const int64_t thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (thread_idx < numel) {
    ...
    for (int64_t level = 0; level < levels; level++) {
      for (ptrdiff_t s = 0; s < S; s++) {
        ...
        scalar_t v = pseudo[e * D * levels + d * levels + level];
        v -= floor(v);
...
//   c10::cuda::MaybeSetDevice(grad_basis.get_device());
  ...
  auto is_open_spline_data = is_open_spline.data_ptr<uint8_t>();
  auto stream = at::cuda::getCurrentCUDAStream();
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(pseudo.scalar_type(), "confidence_basis_bw", [&] {
    auto grad_basis_data = grad_basis.data_ptr<scalar_t>();
    auto pseudo_data = pseudo.data_ptr<scalar_t>();
    auto grad_pseudo_data = grad_pseudo.data_ptr<scalar_t>();
    AT_DISPATCH_DEGREE_TYPES(degree, [&] {
      confidence_basis_bw_kernel<scalar_t, DEGREE>
