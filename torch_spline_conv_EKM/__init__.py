...
for library in ['_version', '_basis' ,'_multispline_basis','_multispline_weighting' , '_multispline_fused' , '_weighting_mod']:
...
cuda_version = torch.ops.torch_spline_conv_EKM.cuda_version()
if torch.version.cuda is not None and cuda_version != -1:  # pragma: no cover
    ...
    if t_major != major:
        raise RuntimeError(
            f'Detected that PyTorch and torch_spline_conv_EKM were compiled with '
            f'different CUDA versions. PyTorch has CUDA version '
            f'{t_major}.{t_minor} and torch_spline_conv_EKM has CUDA version '
            f'{major}.{minor}. Please reinstall the torch_spline_conv_EKM that '
...
from .multispline_weighting import multispline_weighting  # noqa
